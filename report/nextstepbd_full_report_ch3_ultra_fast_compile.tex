...
\node[system] (git) at (2.5,-6) {GitHub\\(Version Control)};
\node[system] (slack) at (8.5,-6) {Slack\\(Internal Comms)};
...
\draw[arrow] (dev) |- node[midway, left=3pt, fill=white, inner sep=1pt] {\scriptsize Push code} (git);
\draw[arrow] (dev) |- node[midway, right=3pt, fill=white, inner sep=1pt] {\scriptsize Discuss} (slack);

% Arrows (Automated Webhooks - dashed)
\draw[arrow, dashed, color=primary] (git) -| node[pos=0.2, below=2pt, fill=white, inner sep=1pt] {\scriptsize Webhook status update} (jira);
\draw[arrow, dashed, color=primary] (slack) -| node[pos=0.2, below=2pt, fill=white, inner sep=1pt] {\scriptsize Chat thread linking} (jira);
