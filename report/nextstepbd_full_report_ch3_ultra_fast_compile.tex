...
\node[system] (jsm) at (4.5,0) {JSM Portal\\(Mandatory Fields)};
\node[actor] (pm) at (9,0) {Project Manager\\(Gatekeeper)};

\node[system] (jira) at (9,-2.5) {Jira Software\\(SSOT / Board)};
\node[actor] (dev) at (4.5,-2.5) {Development\\Team};

\node[system] (git) at (2,-5) {GitHub\\(Version Control)};
\node[system] (slack) at (7,-5) {Slack\\(Internal Comms)};

% Arrows (Direct flows)
\draw[arrow] (client) -- node[above=2pt] {\scriptsize Submit request} (jsm);
\draw[arrow] (jsm) -- node[above=2pt] {\scriptsize Triage queue} (pm);
\draw[arrow] (pm) -- node[right=2pt] {\scriptsize Approve/Convert} (jira);
\draw[arrow] (jira) -- node[above=2pt] {\scriptsize Assign via Lead} (dev);

\draw[arrow] (dev) |- node[above left=2pt] {\scriptsize Push code} (git);
\draw[arrow] (dev) |- node[above right=2pt] {\scriptsize Discuss} (slack);

% Arrows (Automated Webhooks - dashed)
\draw[arrow, dashed, color=primary] (git) -| node[pos=0.15, below=2pt] {\scriptsize Webhook status update} (jira);
\draw[arrow, dashed, color=primary] (slack) -| node[pos=0.15, below=2pt] {\scriptsize Chat thread linking} (jira);

% Return notification to JSM
\draw[arrow, dashed, color=primary] (jira) to[bend right=25] node[right=2pt] {\scriptsize Auto status notification} (jsm);
...
% Bottom-row phases
\node[phase] (p4) at (-6,-7) {4. Testing \& Review\\W9--W10};
\node[phase] (p5) at (0,-7) {5. Training \& Pilot Launch\\W11--W12};
\node[phase] (p6) at (6,-7) {6. Full Deployment \& Optimization\\W13+};
...
% Phase 4 tasks
\node[task] (p4a) at (-6,-8.2) {4.1 Cross-platform integration testing};
\node[task] (p4b) at (-6,-9.15) {4.2 Security and access testing};
\node[task] (p4c) at (-6,-10.1) {4.3 User Acceptance Testing};

% Phase 5 tasks
\node[task] (p5a) at (0,-8.2) {5.1 End-user training};
\node[task] (p5b) at (0,-9.15) {5.2 Pilot project launch};
\node[task] (p5c) at (0,-10.1) {5.3 Feedback collection};

% Phase 6 tasks
\node[task] (p6a) at (6,-8.2) {6.1 Organization-wide rollout};
\node[task] (p6b) at (6,-9.15) {6.2 Monitoring and issue resolution};
\node[task] (p6c) at (6,-10.1) {6.3 Workflow optimization};
