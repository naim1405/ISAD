...
% =================================================================
% ERD-table colour palette and macros (Chapter 5)
% =================================================================
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{images/fig-4-1.png}
    \caption{Current Engineering \& Delivery Workflow}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{images/fig-4-2.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{images/fig-4-17.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{images/fig-4-18.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{images/fig-4-19.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{images/fig-4-20.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{images/fig-4-21.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{images/fig-4-22.png}
...
recommended Configure strategy. Each diagram shows how the new centralized system
replaces the informal, fragmented processes identified in Chapter~4, while keeping
the same external entities in the same positions so the reader can easily trace
the transformation.

The Gane--Sarson DFD notation used here matches the existing-state DFDs of
Chapter~4 for visual consistency. Color coding is applied uniformly:
\textbf{blue} for external entities (unchanged from the existing system),
\textbf{green} for new system processes, and \textbf{yellow} for the new canonical
data store. This color convention applies to every diagram in this section.

% ----------------------------------------------------------------
\subsection{Context Diagram (Level 0) --- New Centralized System}
\label{sec:dfd-context-new}

Figure~\ref{fig:dfd-context-new} shows the Level~0 context diagram of the proposed
new system. Like the existing context diagram (Chapter~4,
Figure~\ref{fig:dfd_context}), it shows four external entities --- Customers (E1),
Management/CEO (E2), Employees (E3), and the HR Department (E4) --- interacting with
a single central process. The difference is that the central process P0 is no longer
labelled as the informal, disconnected existing system. Instead, it is replaced by
a structured, centralized information system that coordinates all business processes
through a canonical data store.

The flow labels on the context diagram are intentionally identical to the existing
context diagram so the reader can see that the same business transactions occur ---
requirements, approvals, reports, task updates --- but that the new system handles
them through structured, auditable, integrated processes rather than informal,
disconnected procedures. The five problem-specific DFDs that follow (Sections~5.2.2
through 5.2.6) show in detail what processes and data stores operate inside this
central system.
...
\begin{tikzpicture}[
    ...
    proc/.style={circle, draw=accent, fill=softteal, very thick, text width=2.8cm, align=center, minimum size=3.2cm},
    flow/.style={-{Latex[length=2.4mm]}, thick, color=primary!80!black},
    flab/.style={font=\footnotesize\sffamily, align=center}
]

% Central process (P0 replaced)
\node[proc] (p0) at (0,0) {\textbf{P0}\\New Centralized Information System};
...
% Customer flows
\draw[flow] ([xshift=-12pt]cust.south) -- ([xshift=-12pt]p0.north)
    node[flab, midway, left] {Requirements,\\Approvals};
\draw[flow] ([xshift=12pt]p0.north) -- ([xshift=12pt]cust.south)
    node[flab, midway, right] {Deliverables,\\Invoices};
...
\caption{Context Diagram (Level 0) of the New Centralized Information System ---
the single process P0 replaces the informal, disconnected system of the existing
diagram with a structured system that coordinates all business processes through a
canonical data store. External entities (E1--E4) are unchanged from the existing
context diagram (Chapter~4, Figure~\ref{fig:dfd_context}).}
\label{fig:dfd-context-new}
\end{figure}

\noindent The context diagram shows that the same four external entities perform the
same categories of transactions, but through a structured system rather than through
disconnected manual procedures. The detail of what processes and data stores operate
inside P0 in the new system is shown in the five problem-specific DFDs that follow.

% ----------------------------------------------------------------
\subsection{New System DFD --- Problem 1: Fragmented Communication \& Collaboration}
\label{sec:dfd-new-prob1}

Figure~\ref{fig:dfd-new-prob1} shows how the new system addresses the fragmented
communication problem (Problem~1). In the existing system, requirements and approvals
passed through informal channels (WhatsApp, email) with no structured capture. The
new system replaces this with a formal intake portal (JSM), a PM triage and approval
workflow, and a canonical store that records every requirement and approval
with full provenance. External entities (Customer, PM, Developer, QA) appear in the
same positions as in the corresponding Chapter~4 DFD so the transformation is
immediately visible.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{images/fig-5-2.png}
    \caption{New System DFD --- Problem 1: Fragmented Communication \& Collaboration.
    Structured requirement intake, PM approval, and canonical record replace
    informal chat-based communication.}
    \label{fig:dfd-new-prob1}
\end{figure}

% ----------------------------------------------------------------
\subsection{New System DFD --- Problem 2: System Fragmentation \& Lack of Integration}
\label{sec:dfd-new-prob2}

Figure~\ref{fig:dfd-new-prob2} shows how the new system addresses system fragmentation
(Problem~2). In the existing system, five disconnected tools (Sheets, GitHub, WhatsApp,
email, docs) held project data in isolation, requiring manual copy-paste between them.
The new system introduces an integration core with an API gateway and webhook handler
that automatically synchronizes data from all connected tools into a single canonical
store. No manual re-entry is required; data changes in one tool are reflected across
the system automatically.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{images/fig-5-3.png}
    \caption{New System DFD --- Problem 2: System Fragmentation \& Lack of Integration.
    An integration core with automated synchronization replaces manual data copying
    across disconnected tools.}
    \label{fig:dfd-new-prob2}
\end{figure}

% ----------------------------------------------------------------
\subsection{New System DFD --- Problem 3: Data Management \& Governance Deficiencies}
\label{sec:dfd-new-prob3}

Figure~\ref{fig:dfd-new-prob3} shows how the new system addresses data management and
governance deficiencies (Problem~3). In the existing system, customer records, project
data, and billing information were maintained in separate disconnected spreadsheets,
requiring manual reconciliation to produce reports. The new system consolidates all
core entities (customer, project, requirement, task, billing) into a single canonical
store, with validation, audit logging, and automated reporting --- eliminating
manual copy-paste and producing consistent, auditable reports from real-time data.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{images/fig-5-4.png}
    \caption{New System DFD --- Problem 3: Data Management \& Governance Deficiencies.
    A single canonical store replaces five disconnected data stores, with automated
    data flow and real-time reporting.}
    \label{fig:dfd-new-prob3}
\end{figure}

% ----------------------------------------------------------------
\subsection{New System DFD --- Problem 4: Security \& Access Control Risks}
\label{sec:dfd-new-prob4}

Figure~\ref{fig:dfd-new-prob4} shows how the new system addresses security and access
control risks (Problem~4). In the existing system, sensitive data (PII, credentials,
contracts) was shared via informal channels (WhatsApp, email attachments) with no
access controls, audit trails, or credential management. The new system introduces
role-based access control (RBAC), a formal audit and compliance process, and secure
credential management. All sensitive data is stored in the canonical store with
appropriate access restrictions and a complete change history.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{images/fig-5-5.png}
    \caption{New System DFD --- Problem 4: Security \& Access Control Risks.
    Secure intake, credential management, RBAC, and audit logging replace informal
    sharing of sensitive data.}
    \label{fig:dfd-new-prob4}
\end{figure}

% ----------------------------------------------------------------
\subsection{New System DFD --- Problem 5: Scalability \& Operational Bottlenecks}
\label{sec:dfd-new-prob5}

Figure~\ref{fig:dfd-new-prob5} shows how the new system addresses scalability and
operational bottlenecks (Problem~5). In the existing system, all requests were routed
through the PM as a single point of control, creating a bottleneck that slowed progress
whenever the PM was unavailable. The new system introduces automated task routing with
capacity-based recommendations, while preserving PM oversight through a manual override
path. The onboarding workflow is also formalized, reducing the time required for new
staff to become productive.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{images/fig-5-6.png}
    \caption{New System DFD --- Problem 5: Scalability \& Operational Bottlenecks.
    Automated task routing with capacity recommendations and a formal onboarding
    workflow remove the single-point-of-failure dependency on the PM.}
    \label{fig:dfd-new-prob5}
\end{figure}
...
specifies exactly how the canonical entities identified in the DFDs of
Section~5.2 are structured, constrained, and related, so that the data model is
internally consistent, supports the workflows described in Section~5.2, and
persists the release and incident evidence created by the new system processes.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{images/fig-5-7.png}
...
observability processes shown in the new system DFDs of Section~5.2.
...
\begin{enumerate}[leftmargin=1.6em]
    \item \textbf{Small canonical core over monolithic build.} The
          integration core (shown in the Level~0 context diagram and the
          Problem~2 system DFD in Section~5.2) is the single piece of
    ...
    \item \textbf{Familiar tooling over disruptive replacement.} The
          configured collaboration and workflow system (shown in the
          Level~0 context diagram and the Problem~1 system DFD in Section~5.2)
