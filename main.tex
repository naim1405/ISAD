...
% NODES
\node[riskbox] (req) at (1, 4.5) {Send Requirement\\(WhatsApp / Email)};
\node[box] (log) at (3.5, 0) {Manual Sorting \&\\Logging (Sheets)};
\node[riskbox] (assign) at (6, 0) {Assign Task\\(WhatsApp Group)};
\node[box] (code) at (8.5, -4.5) {Write Code \&\\Commit (GitHub)};
\node[riskbox] (notify) at (11, -4.5) {Notify Done\\(WhatsApp)};
\node[box] (report) at (11, 0) {Compile Report\\(Excel / Sheets)};
\node[riskbox] (delivery) at (13.5, 4.5) {Final Delivery\\(Email / Slack)};
...
\begin{pgfonlayer}{background}
    \fill[orange!5, rounded corners] (2.2, 1.5) rectangle (12.5, -6);
    \node[orange!80!black, font=\bfseries] at (7.35, -1) {High Dependency \& Fragmentation Zone};
\end{pgfonlayer}
...
\node[step] (client) {Client Request};
\node[step, right=of client] (template) {SRS Drafting};
\node[step, right=of template] (review) {Technical Review};
\node[step, right=of review] (approve) {Client Sign-off};

\node[step, below=1.3cm of approve] (task) {Task Boards};
\node[step, left=of task] (dev) {Development};
\node[step, left=of dev] (test) {QA \& Review};
\node[step, left=of test] (delivery) {Delivery};
